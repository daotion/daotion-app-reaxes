...
export const reaxel_wallet = function(){
	...
	const subscription = wallets$.subscribe( ( [connectedWallet] ) => {
		...
		if(connectedWallet === undefined){
			...
			crayon.green( 'connectedWallet : ' , connectedWallet,connectedWallet.accounts[0].ens );
			globalSetState( {
	...
	let prodiver = Reaxes.observedMemo((first) => {
		if(!globalStore.wallet) return prodiver = null;
		return prodiver = new ethers.providers.Web3Provider(globalStore.wallet?.provider ?? null, 'any');
	} , () => [globalStore.wallet]);
	...
	return () => {
		...
		return {
			...
			get wallet() {
				return globalStore.wallet || null;
			} ,
			get provider (){
				return prodiver;
			},
			...
			address_memoed_reaction(cb:(address:string) => any){
				const lazyInvoke = Reaxes.closuredMemo(() => cb(globalStore.wallet?.accounts?.[0]?.address) , () => []);
				const memo = Reaxes.observedMemo( () => {
					lazyInvoke(() => [globalStore.wallet?.accounts?.[0]?.address])()
		...
		return {
			...
			memo_chain_changing (callback) {
				...
				return Reaxes.observedMemo(() => {
					closured(() => [globalStore.chain])(globalStore.chain);
				},() => [globalStore.chain]);
			},
		};
	};
